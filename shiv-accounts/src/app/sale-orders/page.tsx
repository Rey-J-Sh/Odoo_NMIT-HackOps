...
import { Edit, Eye, Search, FileText, Plus, Users } from 'lucide-react'
...
export default function SaleOrdersPage() {
  ...
  const fetchOrders = async () => {
    try {
      const response = await apiClient.from('sale-orders').select()
        ...
        <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                  ...
                  {paginatedOrders.map((order) => (
                    <tr key={order.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                        {format(new Date(order.so_date), 'MMM dd, yyyy')}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
          ...
          {filteredOrders.length === 0 && (
            <div className="text-center py-12">
              ...
              <h3 className="text-lg font-medium text-gray-900 mb-2">No sale orders found</h3>
